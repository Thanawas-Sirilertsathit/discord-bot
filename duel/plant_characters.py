...
class Greenhouse(Character):
    """Contain greenhouse information (dynamic and static info) and greenhouse methods"""
    def __init__(self):
        super().__init__()
        self.dictionary = dict_finder("Greenhouse")
        self.maxHP = int(self.dictionary["HP"])
        self.HP = int(self.dictionary["HP"])
        self.ATK = int(self.dictionary["ATK"])
        self.name = self.dictionary["Name"]
    ...
    def passive(self, enemy):
        """Passive skill : Herb of knowledge"""
        pass
    ...
    def active_skill(self, enemy):
        """Active skill for greenhouse : Automatic watering system"""
        self.maxHP += 5
        self.HP += 5
